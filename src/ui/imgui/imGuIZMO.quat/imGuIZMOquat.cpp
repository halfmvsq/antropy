...
{
    imguiGizmo g;
    g.modeSettings( ( mode & ( mode ) )  | g.modePanDolly);
    g.posPanDolly = vPanDolly;
...
#ifndef IMGUIZMO_USE_ONLY_ROT
        if(drawMode&modePanDolly || std::abs( io.MouseWheel ) > 1e-6f ) {
            ...
            if((!track.isDollyActive() && !track.isPanActive() && (std::abs( io.MouseWheel ) < 1e-6f) )) q = track.getRotation();
...
#ifndef IMGUIZMO_USE_ONLY_ROT
        else if(highlighted && (std::abs( io.MouseWheel ) > 1e-6f)) getTrackball(qtV);
#endif
